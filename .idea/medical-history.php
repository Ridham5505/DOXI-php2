...
<head>
    ...
    <style>
        ...
        .report-item { padding: var(--spacing-4); margin-bottom: var(--spacing-3); border-left: 4px solid var(--primary-blue); background: var(--gray-50); border-radius: var(--radius-md); }
        .report-title { font-weight: 700; color: var(--gray-900); margin-bottom: var(--spacing-2); }
        .report-meta { font-size: var(--font-size-sm); color: var(--gray-600); margin-bottom: var(--spacing-2); }
        .report-type { display: inline-block; padding: 4px 10px; border-radius: 9999px; font-size: var(--font-size-xs); font-weight: 700; background: var(--light-blue); color: var(--primary-blue); margin-bottom: var(--spacing-2); }
...
<body>
    <div class="page">
        ...
        <div class="card">
            <h2 class="section-title">Medical Records & Reports</h2>
    ...
    <script>
        ...
        async function loadCurrentPatient(){
            ...
            if (res && res.success && res.data){
                ...
                // Extract patient info (medical_records is separate)
                const patient = {...res.data};
                delete patient.medical_records; // Remove medical_records as it's handled separately
                return patient;
            }
        ...
        async function loadMedicalRecords(){
            ...
            if (!res.success){
                container.innerHTML = `<div class="empty">Failed to load medical records.</div>`;
                return;
            }

            const records = res.data?.medical_records || [];
            if (!records.length){
                container.innerHTML = `<div class="empty">No medical records available.</div>`;
                return;
            }
            ...
            records.forEach(r=>{
                ...
                el.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom: var(--spacing-2);">
                        <div class="report-title">${esc(r.title || 'Untitled Record')}</div>
                        <span class="report-type">${esc(type)}</span>
                    </div>
                    <div class="report-meta">Date: ${esc(date)} ${r.doctor_id ? '· Doctor ID: ' + r.doctor_id : ''}</div>
                    ${r.description ? `<div style="color: var(--gray-700); margin-top: var(--spacing-2); line-height: 1.6;">${esc(r.description)}</div>` : ''}
                    ${r.file_path ? `<a href="${esc(r.file_path)}" class="btn btn-outline" style="margin-top: var(--spacing-2);" target="_blank">View File</a>` : ''}
                `;
