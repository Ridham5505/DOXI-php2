        ...
        function displayPatients() {
            ...
            paginatedPatients.forEach(p => {
                ...
                row.innerHTML = `
                    ...
                    <td>${p.phone||'N/A'}</td>
                    <td>${p.last_login || '-'}</td>
                    <td>${typeof p.medical_records_count === 'number' ? p.medical_records_count : (p.medical_records ? p.medical_records.length : 0)}</td>
                    <td>
        ...
        async function viewPatient(patientId) {
            ...
            try {
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-4); margin-bottom: var(--spacing-6);">
                    ...
                    <div>
                        <h4 style="margin: 0 0 var(--spacing-2) 0;">Medical Records</h4>
                        ${Array.isArray(p.medical_records) && p.medical_records.length > 0 ? 
                            `<table style="width: 100%; border-collapse: collapse;">
                                <thead>
                                    <tr>
                                        <th style="text-align: left; padding: var(--spacing-2); border-bottom: 1px solid var(--gray-200);">Date</th>
                                        <th style="text-align: left; padding: var(--spacing-2); border-bottom: 1px solid var(--gray-200);">Type</th>
                                        <th style="text-align: left; padding: var(--spacing-2); border-bottom: 1px solid var(--gray-200);">Title</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${p.medical_records.map(r => `
                                        <tr>
                                            <td style="padding: var(--spacing-2); border-bottom: 1px solid var(--gray-200);">${r.created_at||'-'}</td>
                                            <td style="padding: var(--spacing-2); border-bottom: 1px solid var(--gray-200);">${r.record_type||'-'}</td>
                                            <td style="padding: var(--spacing-2); border-bottom: 1px solid var(--gray-200);">${r.title||'-'}</td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                            </table>` : '<p>No medical records found</p>'}
                    </div>
                </div>`;
